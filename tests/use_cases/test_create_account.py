...
def test_account_creator_run_success_with_selected_institution(
    ...
    with refresh_uow as uow:
        created_account = uow.accounts.get_by_id(account_id)
        created_tags = uow.tags.get_for_account(account_id)
    assert created_account is not None
    assert created_account.institution_id == institution_id
    assert [tag.id for tag in created_tags] == [liquid_id]
...
def test_account_creator_run_success_with_multiple_tags(
    configured_container: Container,
    sample_entities: dict[str, list],
    monkeypatch,
) -> None:
    """Account creation should persist multiple selected tags."""
    init_db_tables_w_entities(configured_container, sample_entities)
    ...
    monkeypatch.setattr(
        nwtrack.entrypoints.cli.adapters.account_presenters,
        "prompt_for_account_description",
        lambda *args, **kwargs: "Brokerage account",
    )
    monkeypatch.setattr(
        nwtrack.entrypoints.cli.adapters.account_presenters,
        "prompt_for_category_choice",
    ...
    assert [tag.id for tag in created_tags] == [liquid_id, core_id]
